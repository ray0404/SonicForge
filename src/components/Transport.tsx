...
export const Transport: React.FC = () => {
  // Use granular selectors to avoid re-renders when currentTime changes
  const isPlaying = useAudioStore(state => state.isPlaying);
  const togglePlay = useAudioStore(state => state.togglePlay);
  const sourceDuration = useAudioStore(state => state.sourceDuration);
  const loadSourceFile = useAudioStore(state => state.loadSourceFile);
  ...
  return (
    <div className="w-full flex items-center justify-between gap-4 max-w-2xl mx-auto">
      ...
      {/* Time & Seeker - Extracted to isolate re-renders */}
      <TransportDisplay />
